...
impl MemView for DebuggerLinuxMemView {
        ...
        if let Some(proc_mem_mtx) = &self.proc_mem {
            ...
            match file.seek(SeekFrom::Start(*addr)) {
                ...
                Err(err) => match err.kind() {
                    ...
                    _ => return Err(MemViewError::ReadAccessDenied),
                },
            };
            file.read_exact(out_data).or(Err(MemViewError::ReadAccessDenied))?;
            *addr += count as u64;
            Ok(())
        } else {
            let pid = self.pid;
    ...
    fn write_bytes(&mut self, addr: &mut u64, value: &[u8]) -> Result<(), MemViewError> {
        ...
        if let Some(proc_mem_mtx) = &self.proc_mem {
            ...
            let bytes = &value;
            file.write_all(bytes).or(Err(MemViewError::WriteAccessDenied))?;
            *addr += count as u64;
            Ok(())
        } else {
            let pid = self.pid;
