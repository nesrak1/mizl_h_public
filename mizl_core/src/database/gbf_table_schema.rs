...
impl GbfTableSchema {
    pub fn new(name: String, key_name: String, key_kind: GbfFieldKind) -> GbfTableSchema {
        GbfTableSchema {
            name,
            key_name,
            key_kind,
    ...
    pub fn read(&self, key: GbfFieldValue, mv: &Box<dyn MemView>, at: &mut u64) -> Result<GbfRecord, MemViewError> {
    ...
    fn read_value(kind: &GbfFieldKind, mv: &Box<dyn MemView>, at: &mut u64) -> Result<GbfFieldValue, MemViewError> {
        ...
        let value = match kind {
            ...
            GbfFieldKind::String => GbfFieldValue::String(read_string(&mv, at)?),
            GbfFieldKind::Bytes => {
